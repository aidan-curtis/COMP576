...
class NeuralNetwork(object):
    ...
    def backprop(self, X, y):
        ...
        dW1 = X.T@((delta3 @ self.W2.T) * self.diff_actFun(self.z1, type=self.actFun_type))
        db1 = np.sum((delta3 @ self.W2.T) * self.diff_actFun(self.z1, type=self.actFun_type), axis=0)
...
class DeepNeuralNetwork(NeuralNetwork):
    def __init__(self, nn_input_dim, nn_hidden_dim, nn_output_dim, n_layers=2 ,actFun_type='tanh', reg_lambda=0.01, seed=0):
        super(DeepNeuralNetwork, self).__init__(nn_input_dim, nn_hidden_dim, nn_output_dim, actFun_type=actFun_type, reg_lambda=reg_lambda, seed=seed)
    ...
    def backprop(self, X, y):
        ...
        (da_prev, self.layers[-1].dW, self.layers[-1].db) = self.layers[-1].backward(delta3, lambda x: 1)
        for l in reversed(range(0, self.n_layers-1)):
            (da_prev, self.layers[l].dW, self.layers[l].db) = self.layers[l].backward(da_prev, lambda x: self.diff_actFun(x, self.actFun_type))
...
def main():
    ...
    model = DeepNeuralNetwork(nn_input_dim=2, nn_hidden_dim=3 , nn_output_dim=2, n_layers=2, actFun_type='relu')
    model.fit_model(X,y)
